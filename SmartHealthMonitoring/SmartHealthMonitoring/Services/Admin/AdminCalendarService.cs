...
namespace SmartHealthMonitoring.Services.Admin
{
    public class AdminCalendarService : IAdminCalendarService
    {
        ...
        public AdminCalendarService(SmartHealthMonitoringContext context)
        {
            _context = context;
        }

        public async Task<AdminCalendarPageViewModel> GetWeekSummaryAsync(DateOnly? weekStart)
        {
            ...
            return new AdminCalendarPageViewModel
            {
                WeekStart = monday,
                WeekEnd = sunday,
                Doctors = doctors,
        ...
        public async Task<List<AdminCalendarEventDto>> GetCalendarEventsAsync(DateTime start, DateTime end, int? doctorId)
        {
            ...
            foreach (var s in blockedSlots)
            {
                ...
                {
                    Id = $"block-{s.Id}",
                    Title = $"Ch?n � {GetShortName(doctorName)}",
                    Start = FormatLocal(displayStart),
                    End = FormatLocal(displayEnd),
        ...
        public async Task<AdminNoShowReportViewModel> GetNoShowReportAsync(DateOnly? from, DateOnly? to)
        {
            ...
            {
                ("Kh�ng d?n (No-show)", totalNoShow, "#ef4444"),
                ("BN hu?", totalCancelPatient, "#f59e0b"),
                ("BS hu?", totalCancelDoctor, "#f97316"),
                ("Ho�n th�nh", totalCompleted, "#10b981"),
                ("�� x�c nh?n", totalConfirmed, "#0ea5e9"),
                ("Kh�c", Math.Max(0, totalOther), "#94a3b8")
            };

            return new AdminNoShowReportViewModel
            {
                FromDate = fromDate,
                ToDate = toDate,
        ...
        public async Task<AdminHeatmapViewModel> GetHeatmapAsync(DateOnly? from, DateOnly? to, int slotMinutes)
        {
            var today = DateOnly.FromDateTime(DateTime.Today);
            ...
            return new AdminHeatmapViewModel
            {
                FromDate = fromDate,
                ToDate = toDate,
                DayLabels = dayLabels,
        ...
        {
            AppointmentStatus.Confirmed => ("�� x�c nh?n", false),
            AppointmentStatus.Pending => ("Ch? duy?t", false),
            AppointmentStatus.CancellationPending => ("Ch? duy?t h?y", false),
            AppointmentStatus.Completed => ("Ho�n th�nh", true),
            AppointmentStatus.NoShow => ("Kh�ng d?n", true),
